...
public class TCPLayer implements BaseLayer {
	...
	public void SetInetSrcAddress(byte[] input) {
		...
		for (int i = 0; i < 4; i++) {
			m_sHeader.TCP_srcaddr.addr[i] = input[i];
		}
	}
	...
	public void SetInetDstAddress(byte[] input) {
		// TODO Auto-generated method stub
		for (int i = 0; i < 4; i++) {
			m_sHeader.TCP_dstaddr.addr[i] = input[i];
		}
	}
	...
	private byte[] ObjToByte(_TCP_Header Header, byte[] input, int length) {
		...
		buf[4] = Header.TCP_srcaddr.addr[0];
		buf[5] = Header.TCP_srcaddr.addr[1];
		buf[6] = Header.TCP_srcaddr.addr[2];
		buf[7] = Header.TCP_srcaddr.addr[3];
